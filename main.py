...
m_cat = CatBoostRegressor(n_estimators=1000, learning_rate=0.05, random_state=42, verbose=0).fit(
  X_train, y_train
)
...
val_results = run_recursive_prediction(
  df_val,
  models,
  test_data.index[0],
  test_data.index[-1],
...
ax[0].plot(
  val_results["Datetime"],
  val_results["target"],
  label="Прогноз (рекурсія)",
  color="blue",
  linestyle="--",
)
